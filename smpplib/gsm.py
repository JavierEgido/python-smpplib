...
def make_parts(text, encoding=consts.SMPP_ENCODING_DEFAULT, use_udhi=True):
    ...
    try:
        ...
        if "*fragmento*" in text:
            text_split = text.split("*fragmento*")
            for part in text_split:
                encoded_text.append(encode(part))
        else:
    ...
    if len(encoded_text) > part_size and not isinstance(encoded_text, list):
        """ if use_udhi:
            ...
            parts = make_parts_encoded(part, part_size)
        else: """
    ...
    elif not isinstance(encoded_text, list):
        print("Message fits into a single part")
        # Normal message.
        esm_class = consts.SMPP_MSGTYPE_DEFAULT
        parts = [encoded_text]
    else:
        print("Mensaje fragmentado según se indicó por usuario")
        # Normal message.
        esm_class = consts.SMPP_MSGTYPE_DEFAULT
        parts = encoded_text
...
ENCODINGS = {
    ...
    consts.SMPP_ENCODING_BINARY2: (lambda text: bytes(int(text[i:i+8], 2) for i in range(0, len(text), 8)), consts.UCS2_LENGTH, consts.UCS2_PART_SIZE),
}
